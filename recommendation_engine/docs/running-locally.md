...
Postgres listens on **5433**, not 5432, so it can't collide with a local Postgres or
creditProxy's stack. The `pgvector/pgvector:pg16` image ships the extension prebuilt.
...
```bash
export RECS_DATABASE_URL="postgresql://recs:recs@localhost:5433/recs"
...
RECS_TEST_DATABASE_URL="postgresql://recs:recs@localhost:5433/recs" pytest tests/ -q
```
